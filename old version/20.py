...
class sonifier:
	...
	def set_frequency(self,freq):
		delta = round(((2*math.pi*freq*(1/self.sample_rate)) % (2*math.pi))*100)
		phase_accumulator = 0
		for s in range(self.n_samples):
			self.sam[s] = round(self.max_sample*self.table[phase_accumulator])
			phase_accumulator += delta
			if phase_accumulator > 314:
				phase_accumulator %= 314
